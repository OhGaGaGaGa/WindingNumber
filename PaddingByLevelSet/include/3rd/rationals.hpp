...
public:
    ...
    template <std::convertible_to<IntType> T>
    rational(const T &n) : num(n), den(1) {}

    template <std::convertible_to<IntType> T, std::convertible_to<IntType> U>
...
#define DEFINE_OPERATOR(OPERATOR, RETURN_TYPE, VALUE, OPPOSITE_VALUE)         \
    ...
    requires std::convertible_to<Arg, rational<IntType>>                      \
...
// Invariant check
template <typename IntType>
bool rational<IntType>::test_invariant() const {
